...
for ieval=1:numel(eval_types)
    ...
    for ifile=1:numel(eval_files) % each test songs
        ...
        [test_wav,fs]=wavread([eI.DataPath,eval_types{ieval},filesep,...
        ...
        [inputforchoose,test_data_cell, target_ag, mixture_spectrum]= ...
                                    formulate_data_choose(mixture,s1,s2, eI, testmode);
        inputch = [inputch inputforchoose];
        load('model_1.mat');
        fprintf('%d\n',ifile);
            [C,pred_cell ] = dataforchoose...
                (theta, eI, test_data_cell, target_ag, mixture_spectrum, true, true);
         drnndata = [drnndata C];
        load('model_2.mat');
         ...
         rtrbmdata = [rtrbmdata C];
            ...
            test_data_gel = dataforcom_test('model_gel',test_data_cell);
            test_data_cla = dataforcom_test('model_cla',test_data_cell);
            test_data_vio = dataforcom_test('model_vio',test_data_cell);
            test_data_pia = dataforcom_test('model_pia',test_data_cell);
%             
            test_data_gac= dataforcom_test('model_gac',test_data_cell);
            test_data_org = dataforcom_test('model_org',test_data_cell);
            test_data_flu = dataforcom_test('model_flu',test_data_cell);
            test_data_tru = dataforcom_test('model_tru',test_data_cell);
% %             
            data_gelNcla = makethedata_test(test_data_gel,test_data_cla);
            clear test_data_gel test_data_cla test_data_cell;
            test_data_gelNcla = dataforcom_test('model_bp_gelNcla',data_gelNcla);
            clear data_gelNcla;
% % %             
            data_vioNpia = makethedata_test(test_data_vio,test_data_pia);
            clear test_data_vio test_data_pia;
            test_data_vioNpia = dataforcom_test('model_bp_vioNpia',data_vioNpia);
            clear data_vioNpia;
% 
              data_gacNorg = makethedata_test(test_data_gac,test_data_org);
              clear test_data_gac test_data_org;
              test_data_gacNorg = dataforcom_test('model_bp_gacNorg',data_gacNorg);
              clear data_gacNorg;
%               
              data_fluNtru = makethedata_test(test_data_flu,test_data_tru);
              clear test_data_flu test_data_tru;
              test_data_fluNtru = dataforcom_test('model_bp_fluNtru',data_fluNtru);
              clear data_fluNtru;
%             
            data_gelclaNviopia = makethedata_test(test_data_gelNcla,test_data_vioNpia);
            clear test_data_gelNcla tet_data_vioNpia;
            test_data_gelclaNviopia = dataforcom_test('model_bprnn_gelclaNviopia',data_gelclaNviopia);
            clear data_gelclaNviopia;
%             
%             
            data_gacorgNflutru = makethedata_test(test_data_gacNorg,test_data_fluNtru);
            clear test_data_gacNorg test_data_fluNtru;
            test_data_gacorgNflutru = dataforcom_test('model_bprnn_gacorgNflutru',data_gacorgNflutru);
            clear data_gacorgNflutru;

            
            test_data_cell = makethedata_test(test_data_gelclaNviopia,test_data_gacorgNflutru);
            clear test_data_gelclaNviopia test_data_gacorgNflutru;            
    ...
    if ieval == 2
        save('inputch.mat','inputch','-v7.3');
        superviseddata = [drnndata;rtrbmdata;comdata];
        save superviseddata.mat superviseddata;
    end
...
%             [Parms_bss3 values]=  BSS_3_EVAL ( s1, s2, wavout_signal, wavout_noise, mixture );
%         else
...
%             wavwrite(wavout_noise, fs, [eI.saveDir,testname,'_ioff',num2str(eI.ioffset),'_nomask_noise.wav']);
%             wavwrite(wavout_signal, fs, [eI.saveDir,testname,'_ioff',num2str(eI.ioffset),'_nomask_signal.wav']);
%             end
%            end
%         else % finish at once
...
%          
%           fprintf('%s\n',eval_files(ifile).name);
%           sSDR(ifile) = values(1,2);aSDR(ifile) = values(1,1);
%           sSIR(ifile) = values(1,2);aSIR(ifile) = values(1,1);
%           sSAR(ifile) = values(1,2);aSAR(ifile) = values(1,1);
%           sNSDR(ifile) = Parms_bss3.NSDR_bss3;
%           aNSDR(ifile) = Parms_bss3.accNSDR_bss3;
% %           rSDR(ifile) = Parms_bss3.SDR_bss3;
% %           rSIR(ifile) = Parms_bss3.SIR_bss3;
% %           rSAR(ifile) = Parms_bss3.SAR_bss3;
% %           rNSDR(ifile) = Parms_bss3.NSDR_bss3;
% 
%          if isfield(eI,'writewav') && eI.writewav==1
%            if exist('stage','var')&& (strcmp(stage,'done')||strcmp(stage,'iter')) % not called by save_callback
%             wavwrite(wavout_noise, fs, [eI.saveDir,testname,'_iter',num2str(iter),'_nomask_noise.wav']);
%             wavwrite(wavout_signal, fs, [eI.saveDir,testname,'_iter',num2str(iter),'_nomask_signal.wav']);
%            end
%          end
% 
%         end
...
% %            if isfield(eI,'writewav') && eI.writewav==1
% %             if exist('stage','var')&& (strcmp(stage,'done')||strcmp(stage,'iter'))
% %                 wavwrite(wavout_noise, fs, [eI.saveDir,testname,'_ioff',num2str(eI.ioffset),'_bmask_noise.wav']);
% %                 wavwrite(wavout_signal, fs, [eI.saveDir,testname,'_ioff',num2str(eI.ioffset),'_bmask_signal.wav']);
% %             end
% %            end
% %         else % finish at once
...
% %             if isfield(eI,'writewav') && eI.writewav==1
% %                if exist('stage','var')&& (strcmp(stage,'done')||strcmp(stage,'iter')) % not called by save_callback
% %                 wavwrite(wavout_noise, fs, [eI.saveDir,testname,'_iter',num2str(iter),'_bmask_noise.wav']);
% %                 wavwrite(wavout_signal, fs, [eI.saveDir,testname,'_iter',num2str(iter),'_bmask_signal.wav']);
% %                end
% %             end
% %         end
% 
...
%    save aSIR.mat aSIR;
%    save aSAR.mat aSAR;
%    save aNSDR.mat aNSDR;
%    save sSDR.mat sSDR;
%    save sSIR.mat sSIR;
%    save sSAR.mat sSAR;
%    save sNSDR.mat sNSDR;
% end
