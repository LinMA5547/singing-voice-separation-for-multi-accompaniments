function  varargout = formulate_data_choose(dmix,s1,s2, eI, mode)
...
windows=sin(0:pi/winsize:pi-pi/winsize);
 ...
 signal =abs(scf* stft(s1, nFFT ,windows, hop, 16000));
 noise =abs(scf* stft(s2, nFFT ,windows, hop, 16000));
 tar = [signal;noise];
multi_data = DATA;
...
theoutputs = {DATA,data_ag, target_ag, mixture_ag};
